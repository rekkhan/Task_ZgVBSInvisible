...
// + Use the basic VBS selection
//==============================
void Select_Event_VBSLead2jets (TreeReader &file_input,   int corMETVer,   int year,   vector<Int_t> &phoIDsel,   vector<Int_t> &jetIDsel,   bool cutBtBPho,   bool cutBtBJet,   vector<long> &nEvent,  vector<double> &nPU)
{
	// + Read the variables
	//---------------------
	int     nVtx        = file_input . GetInt      ("nVtx");
	...
	if (passPhoMET  &&  passJetMET  &&  countFailPhoJet==0)
	{
		passPhoJet = true;
		nEvent[2] += 1;
		nPU[2] += double(nVtx);
	...
	if (jetIDsepMET.size()>1)
	{
		for (unsigned int i=0; i<jetIDsepMET.size()-1; i++)
		{
			massDijet = 0.0;
			v4D_jet1 . SetPtEtaPhiE (jetPt[jetIDsepMET[i]], jetEta[jetIDsepMET[i]], jetPhi[jetIDsepMET[i]], jetEn[jetIDsepMET[i]]);
	...
	if ((!passPhoMET) || (!passJetMET) || (!passPhoJet) || (!passJetJet)  ||  invPt<120)
	{
		phoIDsel . clear();
		jetIDsel . clear();
	}
